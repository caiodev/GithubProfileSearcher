...
import githubprofilesearcher.caiodev.com.br.githubprofilesearcher.datasource.states.Neutral
...
class RemoteFetcher {
    suspend inline fun <reified T> call(
        call: () -> HttpResponse,
        crossinline isCoroutineActive: () -> Boolean,
    ): State<*> =
        runCatching { call() }
            .fold(
                onSuccess = { content ->
                ...
                onFailure = { exception ->
                    if (exception is CancellationException) {
                        if (!isCoroutineActive()) Generic else Neutral
                    } else {
